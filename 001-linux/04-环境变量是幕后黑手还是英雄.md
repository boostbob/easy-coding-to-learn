当我们输入 uptime 命令后，Shell 怎么知道这个命令在哪里呢? 因为 Shell 有个上下文环境，它会在预定的目录里去查找命令，这些预定的目录可以输入 echo $PATH 来获取，PATH 就是一个环境变量，它定义了命令搜索的路径和顺序。如果两个目录里都有一个叫 uptime 的命令，那输入的是哪一个呢? 使用 which 命令来确定。
