...
函数调用的时候编译器会计算函数需要使用的栈空间，记录当前函数下一条指令执行的位置、局部变量、调用参数、要执行的函数指令地址，记录的意思就是把它们压入执行栈，被调用的函数指令执行完了后再跳回到原来指令的位置(先进后出)，继续执行原来函数的下一条指令，层层调用直到  main 函数执行完毕退出，而且栈空间大小是不断调整的。
